(function(proxy) {
    ...
    var PREPARE_LINK_HANDLERS = {
        ...
        'get image': {
            ...
            handle: function(baseUri, link, cb) {
                ...
                iframely.getImageMetadata(uri, function(error, data) {
                    if (error) {
                        link._imageMeta = {
        ...
        async.waterfall([
            ...
            function(cb) {
                ...
                if (_.intersection(iframelyProvides, methodsData._allRequirements).length > 0) {
                    ...
                    iframely.getPageData(uri, opts, cb);
                } else {
                    cb(null, null);
                }
            },
    ...
    * */
    ...
    function scanModulesForPlugins() {
        ...
        var modules_listing = fs.readdirSync(modulesRootPath);
        modules_listing.forEach(function(module_name) {

            // Check if module is plugin container.
            var modulePackagePath = path.resolve(modulesRootPath, module_name, 'package.json');
            ...
            if (fs.existsSync(modulePackagePath)) {
                ...
                var modulePluginsPath = path.resolve(modulesRootPath, module_name, 'plugins', 'domains');
                if (fs.existsSync(modulePluginsPath)) {
                    scanAllPluginsDir(modulePluginsPath);
                }

                var modulePluginsPath = path.resolve(modulesRootPath, module_name, 'plugins', 'generic');
                if (fs.existsSync(modulePluginsPath)) {
                    scanAllPluginsDir(modulePluginsPath);
                }

                var modulePluginsPath = path.resolve(modulesRootPath, module_name, 'plugins', 'custom');
                if (fs.existsSync(modulePluginsPath)) {
                    scanAllPluginsDir(modulePluginsPath);
                }

                // Scan template plugins (just one more dir for organisation).
                var moduleTemplatePluginsPath = path.resolve(modulesRootPath, module_name, 'plugins', 'templates');
        ...
        pluginsList.sort(function(p1, p2) {
            var pr1 = p1.module.lowestPriority;
            var pr2 = p2.module.lowestPriority;

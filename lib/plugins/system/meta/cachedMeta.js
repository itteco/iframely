        ...
        if (options.refresh || options.cache_ttl === 0) {
            ...
            function callback(error, data) {
                ...
                if (!error && data) {
                    ...
                    // Possible errors: `redirect` or `responseStatusCode`.
                    // Sometimes meta can contain `meta.error` as usual meta field, so check only specific fields.
                    if ((data.error?.redirect && data.error?.status) || data.error?.responseStatusCode) {
                        ...
                        log(`   -- Using cached htmlparser error for: "${url}"; data=${JSON.stringify(data)}`);
                        cb(data.error);

...
var defaultMediaKeys = {'audio': 1, 'image': 1, 'video': 1};

function merge(parentObj, props, value) {

    function getDefaultKey(parent) {
        if (defaultMediaKeys[parent]) {
            return 'url';
        } else {
            return 'value';
        }
    ...
    // Each video:url tag have to create new object in array.
    if (props.length > 1 && (currentNodeName === 'value' || currentNodeName === 'url')) {
    ...
    if (!(currentNodeName in parentNode)) {
        ...
        //console.log(currentNode, currentChild, parentNode, props, key);

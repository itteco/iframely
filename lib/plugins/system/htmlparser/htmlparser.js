...
export default {
    ...
    getData: function(url, whitelistRecord, options, __noCachedHtmlparserFallback, cb) {
        ...
        getUrlFunctional(url, options2, {
            ...
            onResponse: function(resp, request_options) {
                ...
                // Filter request headers starting with "x-".
                var filtered_request_headers = Object.fromEntries(
                    Object.entries(request_options.headers || {})
                        .filter(([key]) => key.toLowerCase().startsWith('x-'))
                );

                headers.request_headers = filtered_request_headers;

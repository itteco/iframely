...
module.exports.getOembed = function(uri, options, callback) {
    ...
    try {
        ...
        if (params) {
            ...
            var urlObjForCache = urlLib.parse(uriForCache, true, true);
            var query = urlObj.query;
            var queryForCache = urlObjForCache.query;
            delete urlObj.search;
            delete queryForCache.search;

            Object.keys(params.params).forEach(function(key) {
                if (/^\$\{.+\}$/.test(params.params[key])) {
                    var token = options.getRequestOptions(params.params[key].replace(/^\${/, '').replace(/}$/, ''), '');
                    // Do not cache `SYSTEM_TOKENS`.
                    if (!CONFIG.SYSTEM_TOKENS || CONFIG.SYSTEM_TOKENS.indexOf(token) === -1) {
                        queryForCache[key] = token;
                    }
                    query[key] = token;
                } else {
                    query[key] = params.params[key];
                    queryForCache[key] = params.params[key];
                }
            });

            uri = urlLib.format(urlObj);
            uriForCache = urlLib.format(queryForCache);
        }
    } catch(ex) {
        console.error("Error using ADD_OEMBED_PARAMS", ex);
    }

    var oembed_key = 'meta:' + uriForCache;

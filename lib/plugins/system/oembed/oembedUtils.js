...
module.exports.getOembed = function(uri, options, callback) {
    ...
    var token_check = CONFIG.SYSTEM_TOKENS && CONFIG.SYSTEM_TOKENS.find(key => oembed_key.indexOf(key) > -1);
    if (token_check) {
        oembed_key = oembed_key.replace('access_token='+ token_check, '').replace('&&', '').replace(/&$/, '');
    }    

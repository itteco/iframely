...
module.exports.getOembed = function(uri, options, callback) {
    ...
    async.waterfall([
        ...
        function(data, cb) {
            ...
            var cachedError = data && (data.response && data.data && data.data.error) || data.error;
            if (cachedError) {
            ...
            if (data && !cachedError) {
                ...
                if (/^https:\/\/graph\.facebook\.com\/v8\.0\/oembed/.test(uri)) {
                    sysUtils.log('   -- Using cached oembed for: ' + uri, JSON.stringify(data));
                } else {

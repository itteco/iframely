...
module.exports = {
    ...
    getData: function(url, oembedLinks, options, cb) {
        ...
        if (skip && !options.promoUri && !options.forcePromo) {
            var parsed = urlLib.parse(href, true);
            if (parsed.query.promo && parsed.query.promo !== '0' && parsed.query.promo !== 'false') {
                // Do not skip parsing promo oembed if calling from non promo request.
                skip = false;
            }
        }

    ...
    prepareLink: function(url, link, options, pluginContext, cb) {
        ...
        function setLinkType(link, error, data) {
            ...
            if (data.type == CONFIG.T.text_html && data.x_frame_options && /^sameorigin|deny$/i.test(data.x_frame_options)) {
                link.error = 403;
                link.message = 'iFrame removed due to x-frame-options header';
            }

            if (data.type == 'application/vnd.apple.mpegurl' || data.type == 'application/x-mpegURL') {
                link.type = 'video/m3u8';
                data.type = 'video/m3u8';
            }

            if (data.type == 'vide/m3u8' && (!data.access_control || data.accesss_control.allow_origin !== '*')) {
                link.error = 403;
                link.message = 'CORS headers on the player are not configured correctly';
            }

            if ((link.type == CONFIG.T.maybe_video && !(/^video\//i.test(data.type))) || (link.type == CONFIG.T.maybe_audio && !/^audio\//i.test(data.type))) {

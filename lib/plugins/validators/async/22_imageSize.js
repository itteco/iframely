...
module.exports = {
    ...
    prepareLink: function(url, link, options, iterationPluginContext, cb) {
        ...
        if (!match) {
            cb();
            // do the rest asynchrounously - for the next time
        }
        ...
        utils.getImageMetadata(href, _.extend({}, options, {multiCache: iterationPluginContext.multiCache}), function(error, data) {
            ...
            if (match) { 
                // otherwise it was called already above
                cb();
            }
        });
    }

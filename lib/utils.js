...
export function getMaxCacheTTL(url, options, default_min_ttl, ttl_options) {
    ...
    // Use `proxy.cache_ttl` if requested.
    if (ttl_options 
        && ttl_options.proxy_ttl_priority
        && proxy
        && proxy.cache_ttl >=0
        && (!result || result > proxy.cache_ttl)) {
            result = proxy.cache_ttl;
    }
 ...
 * */
 export function getImageMetadata(uri, options, callback){
    ...
    cache.withCache("image-meta:" + uri, function(callback) {
        ...
        ttl: getMaxCacheTTL(uri, options, CONFIG.IMAGE_META_CACHE_TTL, {proxy_ttl_priority: true}),

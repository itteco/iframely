...
var prepareRequestOptions = exports.prepareRequestOptions = function(request_options, options) {
    ...
    var lang = options && options.getProviderOptions && options.getProviderOptions('locale');
    if (lang) {
        request_options.headers = request_options.headers || {};
        request_options.headers['Accept-Language'] = lang + CONFIG.ACCEPT_LANGUAGE_SUFFIX;
    }
        ...
        try {
            ...
            var r = request({
                ...
                headers: {
                    ...
                    'Connection': /^https:\/\//i.test(url) ? 'keepalive' : 'close',
                },
                maxRedirects: options.maxRedirects || 3,
                timeout: options.timeout || CONFIG.RESPONSE_TIMEOUT,
                followRedirect: options.followRedirect,
                    ...
                    .on('response', function(res) {
                        ...
                        if (content_type && content_type !== 'application/octet-stream' && content_type !== 'binary/octet-stream') {
                            ...
                            if (!uri.match(/\.(jpg|png|gif)(\?.*)?$/i)) {
                                return cb('invalid content type: no content-type header and file extension');
                            }
                        }
    ...
    cache.withCache("content-type:" + uriForCache, function(cb) {
        ...
        getHead(uriOriginal, {
            ...
            .on('response', function(res) {
                ...
                if (!error && content_type == 'text/html' && /^sameorigin|deny$/i.test(res.headers['x-frame-options'])) {
                    error = 403;
                }

                finish(error, content_type);
            })
            .on('error', function(error) {
                finish(error);
            });
...
function getWhitelistLogData(meta, oembed) {
    ...
    if (meta) {
        ...
        if (meta.ld && meta.ld.VideoObject) {
            r.embedURL = !!(meta.ld.VideoObject.embedURL || meta.ld.VideoObject.embedUrl);
        }

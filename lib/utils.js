...
import { Http2Agent } from './agent-http2.js';
...
function prepareFetchHeaders(headers) {
    var result = {};
        ...
        try {
            ...
            fetch(request_options.url || request_options.uri, request_options)
                .then(res => {
                    ...
                    if (!options.asBuffer) {
                        res.body.setEncoding("binary");
                    }

                    // TODO:
                    var body = res.body;
                    body.status = res.status;
                    body.headers = prepareFetchHeaders(res.headers.raw());

                    // TODO:
                    req.emit('response', body);
                })
                .catch(error => {
...
export function getHeadFunctional(url, options, callbacks) {
    ...
    getHead(url, options)
        ...
        .on('response', function(response) {
            ...
            body.headers = prepareFetchHeaders(response.headers.raw());

            callbacks.onResponse && callbacks.onResponse(body);
        });
}
        ...
        fetch(request_options.uri || request_options.url, request_options)
            .then(res => {
                ...
                if (options.checkHeaders) {
                    data.headers = prepareFetchHeaders(res.headers.raw());
                }
                cb(null, data);
            })
            .catch(cb)

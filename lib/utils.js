...
import { context, AbortController } from 'fetch-h2';
const { fetch } = context( );
...
export function getUrlFunctional(url, options, callbacks) {
    ...
    getUrl(url, options)
        .on('error', function(error, opts) {
            ...
            if (!options.secondAttempt && (error.code in CONFIG.HTTP2_RETRY_CODES)) {
                log('   -- getUrl ' + error.code + ' first', opts, url);
                ...
                log('   -- getUrl ' + error.code + ' second', opts, url);
            }

            callbacks.onError && callbacks.onError(error);
        })
        .on('abortController', function(abortController) {
            callbacks.onAbortController && callbacks.onAbortController(abortController);
        })
 ...
 export function getUrl(url, options) {
    ...
    process.nextTick(function() {
        try {
            ...
            // TODO: review node-fetch options
            var request_options = prepareRequestOptions({
                ...
                headers: {
                    ...
                    'Accept': '*/*'
                    // gzip, br, etc - set by `node-fetch` `compress: true` option.
                },
                ...
                follow: options.followRedirect ? (options.maxRedirects || CONFIG.MAX_REDIRECTS) : 0
            }, options);

            fetch(request_options.url || request_options.uri, request_options)
                .then(res => {
                    response = res;
                    return res.readable();
                })
                .then(stream => {
                    ...
                    stream.headers = response.headers.toJSON();

                    // TODO:
                    req.emit('response', stream);
                })
                .catch(error => {
                    req.emit('error', error, {http2Agent: false/*request_options.agent === http2Agent*/});
                });


            req.emit('abortController', controller);
...
export function getHeadFunctional(url, options, callbacks) {
...
export function sendLogToWhitelist(uri, context) {
    ...
    if (data) {
        ...
        fetch(CONFIG.WHITELIST_LOG_URL, {
            method: 'GET',
            qs: data
        })
            .then(res => {
...
var getUriStatusPrivate = function(uri, options, cb) {
    ...
    try {
        fetch(request_options.uri || request_options.url, request_options)
            .then(res => {
                controller.abort();

                var data = {
                    code: res.status,
                    content_type: res.headers && res.headers.get('content-type'),
                    content_length: res.headers && res.headers.get('content-length') ? parseInt(res.headers.get('content-length') || '0', 10) : null
                };
                if (options.checkHeaders) {
                    data.headers = res.headers.toJSON();
                }
                cb(null, data);
            })
            .catch(cb)
...
function isInterestingPlayer(data, canonical) {
    if (typeof data !== 'string' && !(data.url || data.value)) {
        return false;
    }

    var url = data && (data.url || data);

    if (/\.(mp4|m4v|m3u8|mp3)(\?.*)?$/i.test(url)
        || /^(https?:)?\/\/\/?$/.test(url)) {
        return false;
    }
...
function getWhitelistLogData(meta, oembed) {
    ...
    if (meta) {
        ...
        var canonical = meta.canonical || meta.og && meta.og.url;
        ...
        */
        ...
        r.twitter_stream =
            meta.twitter && meta.twitter.player && !!meta.twitter.player.stream && isInterestingPlayer(meta.twitter.player.stream, canonical);

        r.og_video =
            !!meta.og && !!meta.og.video
            && isInterestingPlayer(meta.og.video, canonical);
        ...
        if (ld) {
            ...
            if (json) {
                ...
                r.embedURL = !!embedURL && isInterestingPlayer(embedURL, canonical);
            }
        }

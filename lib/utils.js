...
export function prepareRequestOptions(request_options, options) {
    return new Promise((resolve, reject) => {
        try {
            ...
            var enable_domain_prerender = options?.getDomainOptions && options.getDomainOptions('robot.prerender') || options?.prerender;
            if (enable_domain_prerender) {
                setPrerender(enable_domain_prerender);
            }

            // Make empty headers by default for all cases.
            request_options.headers = request_options.headers || {};

            // Put forward headers.
            const forward_headers = options?.getProviderOptions && options.getProviderOptions('app.http_forward_headers');
            if (forward_headers && typeof forward_headers === 'object' && !Array.isArray(forward_headers)) {
                for (const [key, value] of Object.entries(forward_headers)) {
            ...
            if (options?.getProviderOptions 
                ...
                if (ext && /^[a-zA-Z0-9\.\s_-]+$/.test(ext) && ext.length >= 2) {
                    request_options.headers['User-Agent'] += ` ${ext[0].toUpperCase()}${ext.slice(1)}`;

                    var ua_url = options.getProviderOptions('app.ua_url');
                    if (ua_url) {
                        request_options.headers['User-Agent'] += ` (+${ua_url})`;
                    }
                }
            }
            ...
            // Custom robot auth headers.
            if (options?.getDomainOptions) {
                const robot_header_name = options.getDomainOptions('robot.header_name');
                const robot_header_value = options.getDomainOptions('robot.header_value');
                if (robot_header_name && robot_header_value) {
                    request_options.headers[robot_header_name] = robot_header_value;
                }
            }
            ...
            // Has header begining from "Signature".
            const skip_sig = Object.keys(request_options.headers).some(key => /^signature/i.test(key));

            if (!skip_sig && (options?.sig || request_sig) && options?.getSigHeaders) {
                try {
        ...
        prepareRequestOptions({
            ...
            .then(stream => {
                ...
                if (options.followRedirect 
                    ...
                    redirect_options.jar = extendCookiesJar(url, redirect_options.jar, stream.headers);                    

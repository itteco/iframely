...
function getCustomProxyForUri(uri) {
...
export function prepareRequestOptions(request_options, options) {
    ...
    if (CONFIG.PROXY || (options && options.proxy)) {
        ...
        var proxy = (options && options.proxy) || getCustomProxyForUri(uri);
        if (proxy) {
    ...
    if (options && options.getProviderOptions 
        ...
        var ext = options.getProviderOptions('app.name', options.getProviderOptions('app.ua_extension'));
        if (ext.length > 1) {
 ...
 */
 export function getUrl(url, options, callbacks) {
    ...
    var request_options = prepareRequestOptions({
        ...
        redirect: 'manual', // Always manual, inner redirects handled with cookies by custom logic.
        follow: 0
    }, options);
    ...
    try {
        fetchStreamKeepAlive(request_options)
            .then(stream => {
                ...
                if (options.followRedirect && stream.status >= 300 && stream.status < 400 && stream.headers.location) {

                    if (options.maxRedirects <= 0) {
                        throw new Error('max redirects limit reached');
                    }

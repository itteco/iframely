...
exports.getContentType = function(uri, options, cb) {

    cache.withCache("content-type:" + uri, function(cb) {
        ...
        getHead(uri, {
            ...
            .on('response', function(res) {
                finish(null, res.headers['content-type']);
            })
            .on('error', function(error) {
                finish(error);
            });

    }, cb, options.disableCache);
};
...
exports.generateLinksHtml = function(data, options) {
    ...
    var mainLink = htmlUtils.findMainLink(data, options);
    if (mainLink) {
        if (mainLink.html) {

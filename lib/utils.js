    ...
    getUrl(url, options)
        .on('error', function(error) {
            ...
            if (!options.secondAttempt && (error.code in CONFIG.HTTP2_RETRY_CODES)) {
                sysUtils.log('   -- getUrl ' + error.code + ' first', isHttp2Agent ? 'http2Agent' : 'http1', url);
                ...
                sysUtils.log('   -- getUrl ' + error.code + ' second', isHttp2Agent ? 'http2Agent' : 'http1', url);
            }

            callbacks.onError && callbacks.onError(error);
        })
        .on('request', function(request) {
            if (request.agent === http2Agent) {
                isHttp2Agent = true;
            }

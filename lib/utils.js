...
var getUrlFunctional = exports.getUrlFunctional = function(url, options, callbacks) {
    ...
    getUrl(url, options)
        .on('error', function(error) {
            ...
            // Retry on ECONNRESET.
            if (!options.secondAttempt && error.code === 'ECONNRESET') {
                sysUtils.log('   -- getUrl ECONNRESET first', url);
                process.nextTick(function() {
                    getUrlFunctional(url, _.extend({}, options, {secondAttempt: true}), callbacks);
                });
                return;
            } else if (options.secondAttempt && error.code === 'ECONNRESET') {
                sysUtils.log('   -- getUrl ECONNRESET second', url);
            }
...
var getHeadFunctional = exports.getHeadFunctional = function(url, options, callbacks) {
    ...
    getHead(url, options)
        .on('error', function(error) {
            ...
            // Retry on ECONNRESET.
            if (!options.secondAttempt && error.code === 'ECONNRESET') {
                sysUtils.log('   -- getHead ECONNRESET first', url);
                process.nextTick(function() {
                    getHeadFunctional(url, _.extend({}, options, {secondAttempt: true}), callbacks);
                });
                return;
            } else if (options.secondAttempt && error.code === 'ECONNRESET') {
                sysUtils.log('   -- getHead ECONNRESET second', url);
            }
    ...
    cache.withCache("image-meta:" + uri, function(callback) {
        ...
        async.waterfall([
            function(cb) {
                ...
                function finishCb(error, data) {
                    // imagesize may call callback twice for http2.
                    if (imagesizeDone) {
                        console.error('  -- Double callback call on getImageMetadata for', uri, error, data)
                        return;
                    }
                    imagesizeDone = true;
                    cb(error, data);
                    ...
                    onResponse: function(res) {
                        ...
                        if (content_type && content_type !== 'application/octet-stream' && content_type !== 'binary/octet-stream') {
                            ...
                            if (content_type.indexOf('image') === -1 && !uri.match(/\.(jpg|png|gif)(\?.*)?$/i)) {
                                return finishCb('invalid content type: ' + res.headers['content-type']);
                            }
                        }
                        ...
                        if (res.statusCode == 200) {
                            ...
                            imagesize(res, finishCb);
                        } else {
                            finishCb(res.statusCode);
                        }
                    },
        ...
        function makeCall(method) {
            ...
            methodCaller(uriOriginal, {
                ...
                onResponse: function(res) {
                    ...
                    if (!method && (error === 405 || error === 403 || error === 400 || error >= 500)) {

                        // Method HEAD is not allowed. ex. Amazon S3 (=403)
                        // Try call with GET.
                        makeCall('GET');
                        return;
                    }

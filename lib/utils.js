 ...
 * @public 
 * @param {Object} [options] Options.
 * @param {Boolean} [isMax] Condition to check if the max embed varitation is already used
 * @param {Boolean} [isMin] Condition to check if the min embed varitation is already used
 * @param {Object} [messages] More/Less messages to use:
 *  - messages.min for minimum varation, null if not supported for current URL
 *  - messages.max for maximum varation, null if not supported for current URL
 *  - messages.default for mediaum varation, null if not supported for current URL
 * */
exports.getVary = function(options, isMax, isMin, messages) {

    var vary = {};
    var isMore = options.getProviderOptions(CONFIG.O.more);
    var isLess = options.getProviderOptions(CONFIG.O.less);

    if (isMax) { // then check how it was achieved

        if (!isMore) {  // max'ed via user's default settings
            vary[CONFIG.O.less] = messages.default || messages.min; // then announce an option to go down
        } else {        // maxed via iframely=more, add the message
            vary[CONFIG.O.more] = messages.max;
        }
    } else if (isMin) { // check what made a min
        if (isLess) { // min'ed via iframely=less, add the message
            vary[CONFIG.O.less] = messages.min;
        } else { // min'ed via user's default settings, add more message
            vary[CONFIG.O.more] = messages.default || messages.max;
        }
    } else { // the mid version. can be default or upped/downed to by more/less
        if (isMore) { // mid version upped from min
            vary[CONFIG.O.more] = messages.default;
        } else if (isLess) { // downed from max by less
            vary[CONFIG.O.less] = messages.default;
        } else { // mid version is default
            if (messages.min) {
                vary[CONFIG.O.less] = messages.min;
            }
            if (messages.max) {
    ...
    // accept null min/max messages to signal that those options are not supported for particular URL
    if (vary[CONFIG.O.less] === null) { delete vary[CONFIG.O.less];}
    if (vary[CONFIG.O.more] === null) { delete vary[CONFIG.O.more];}

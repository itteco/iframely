    ...
    * */
    ...
    function getCharset(string, doNotParse) {
        ...
        if (doNotParse) {
            ...
            var attrs = {};
            string.split(";").slice(1).forEach(function (attr) {
                attr = attr.split("=");
                attrs[attr[0].trim().toLowerCase()] = attr.slice(1).join("=").trim();
            });

            if (attrs.charset) {
                charset = attrs.charset.toUpperCase();
            }
        }

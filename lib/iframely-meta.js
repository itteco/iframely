    ...
    * */
    iframely.getImageMetadata = function(uri, options, callback){
        ...
        withCache("image:" + uri, function(cb) {
            ...
            var loadImageHead, parseImageSize, totalTime;
    ...
    function encodeText(charset, text) {
        try {
            var encoded = iconv.encode(text, 'ISO8859-1');

            if (charset) {
                return iconv.decode(encoded, charset).toString();
     ...
     * @private
     */
    function withCache(key, func, callback, disableCache) {

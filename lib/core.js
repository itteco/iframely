    ...
    * */
    ...
    function generateProviderOptionsFunc(url, options) {
        if (options.returnProviderOptionsUsage) {
            ...
            options._usedDomainOptions = options._usedDomainOptions || [];
        }
        ...
        function registerDomainOptionsUsage(path) {
            if (options.returnProviderOptionsUsage) {
                if (options._usedDomainOptions.indexOf(path) === -1) {
                    options._usedDomainOptions.push(path);
                }
            }
        }
        ...
        options.getDomainOptions = function(path, defaultValue) {
            registerDomainOptionsUsage(path);
            var bits = path.split('.');
            var value = searchParamInObj(0, bits, options.domainOptions);
            return typeof value !== 'undefined' ? value : defaultValue;
        };
    ...
    * */
    export function run(uri, options, cb) {
        ...
        if (options.getDomainsOptions) {
            options.domainOptions = options.getDomainOptions(uri);
        }

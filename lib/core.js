    ...
    * */
    ...
    var SYS_ERRORS = {
        ...
        timeout: 'timeout',
        fallback: 'fallback',
        recall: 'recall'        // Make same iframely call with different options.
    };
    ...
    function findRecallError(result) {
        if (result) {
            for(var i = 0; i < result.length; i++) {
                var r = result[i];
                var recall = r.error && r.error[SYS_ERRORS.recall];
                if (typeof recall === "object") {
                    log('   -- plugin recall (by "' + r.method.pluginId + '")');
                    return recall;
                } else if (recall && typeof recall !== "object") {
                    log('   -- skip plugin recall, not object (by "' + r.method.pluginId + '")', recall);
                }
            }
        }
    }
    ...
    * */
    export function run(uri, options, cb) {
        ...
        // Store original to run on recall.
        ...
        var requiredPlugins = pluginsSet.initialPlugins,
            ...
            asyncMethodCb = function(error, result) {
                ...
                // Find recall command.
                var recall_data = findRecallError(result);
                if (recall_data && options.isRecall) {
                    log('   -- ignore recursive recall');
                } else if (recall_data) {
                    abortCurrentRequest();
                    var recallOptions = Object.assign({}, originalOptions, recall_data, {isRecall: true});
                    run(uri, recallOptions, cb);
                    aborted = true;
                    return;
                }

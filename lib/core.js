(function(core) {
    ...
    * */
    var run = core.run = function(uri, options, cb) {
        ...
        function requestWrapperWithParams(r_options, callback) {
            // Override 'refresh' only if present from caller and not present in plugin.
            if (!('refresh' in r_options) && ('refresh' in options)) {
                r_options.refresh = options.refresh;
            }
            requestWrapper(r_options, callback);
        }

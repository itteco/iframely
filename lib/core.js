    ...
    * */
    ...
    function resultsHasDomainData(requiredPlugins, allData) {
        ...
        for(var i = 0; i < allData.length && !hasDomainData; i++) {
            ...
            if (plugin.domain && (r.method.name === 'getLink' || r.method.name === 'getLinks') && r.data && !r.error) {
                hasDomainData = true;
            }
        }
    ...
    function prepareResultData(uri, result, options) {
        ...
        for(var i = 0; i < links.length;) {
            ...
            // Remove canonical links.
            if (canonical && link.href && typeof link.href === 'string' && link.rel.indexOf(CONFIG.R.file) === -1 && link.rel.indexOf(CONFIG.R.iframely) === -1) {

    ...
    * */
    ...
    const ALLOWED_DEBUG_DATA = ['meta', 'ld', 'og', 'twitter', 'oembed'];
    ...
    function prepareResultData(uri, result, options) {
        ...
        if (options.debug_sources) {
            ...
            result.allData.forEach(function(d) {
                if (d.data && d.method.name === 'getData') {
                    ...
                    // TODO: all listed temporary
                    if (true || plugin.listed) {
                        for(var key in d.data) {
                            if (ALLOWED_DEBUG_DATA.indexOf(key) > -1) {
                                result._data[key] = d.data[key];
                            }
                        }
                    }
                }
            ...
            result._redirects = options.redirectsHistory || [];
        }
        ...
        function getSourcesParams(_sources, _sources_params, method_name) {
            for(var key in _sources) {
                ...
                _sources_params[key] = params.filter(param => param in result._data);
            }
        }

        if (options.debug_sources) {
            result.meta._dependencies = result.meta._dependencies || {};
            result.vars._dependencies = result.vars._dependencies || {};
            getSourcesParams(result.meta._sources, result.meta._dependencies, 'getMeta');
            getSourcesParams(result.vars._sources, result.vars._dependencies, 'getVars');
        }
        ...
        if (result.vars._dependencies && Object.keys(result.vars._dependencies).length === 0) {
            delete result.vars._dependencies;
        }

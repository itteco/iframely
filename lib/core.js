    ...
    function isGetVarsMethod(name) {
        return name === "getVars" || CONFIG.GET_VARS_METHODS && name in CONFIG.GET_VARS_METHODS
    }

    /*
     * Recursively finds plugin methods to run, including 'mixins' and dependencies (up and down by tree).
     * */
    function findPluginMethods(opts) {
        const {pluginId, loadedParams, pluginsUrlMatches, usedMethods, usedParams, methods, scannedPluginsIds, usedDomains, getVarsInfo, mandatoryParams} = opts;
        ...
        if (!mandatoryParams) {
            ...
            if (mixins) {
                for(var i = 0; i < mixins.length; i++) {
                    findPluginMethods({...opts, pluginId: mixins[i]});
                }
            }
        }
            ...
            if (method in plugin.methods) {
                ...
                if (absentParams.length > 0) {
                    ...
                    for(var j = 0; j < pluginsId.length; j++) {
                        ...
                        findPluginMethods({...opts, pluginId: foundPluginId, mandatoryParams: undefined});
                    }

                } else {

                    if (!getVarsInfo.lastIteration && isGetVarsMethod(method)) {

                        getVarsInfo.pendingMethods = true;
        ...
        for(var i = 0; i < methods.length; i++) {
            (function() {
                ...
                try {
                    ...
                    // Call method.
                    // console.log('-- run method:', method.pluginId, method.name)
    ...
    * */
    function runPluginsIteration({requiredPlugins, context, pluginsUrlMatches, usedMethods, usedParams, usedDomains, getVarsInfo, options, asyncMethodCb}) {
        ...
        for(var i = 0; i < requiredPlugins.length; i++) {
            ...
            findPluginMethods({pluginId: plugin.id, loadedParams, pluginsUrlMatches, usedMethods, usedParams, methods, scannedPluginsIds, usedDomains, getVarsInfo});
        }
    ...
    * */
    ...
    function useResult(usedMethods, context, pluginsContexts, allResults, result, options, asyncMethodCb) {
        ...
        for(var i = 0; i < result.length; i++) {
            ...
            if (r.data) {
                    ...
                    for(var key in r.data) {
                        ...
                        if (key === 'message') {
                            ...
                            if (!(key in context) || key === 'whitelistRecord') {
                                context[key] = dataValue;
                                hasNewData = true;
                            } else {
                                // Make arrays from `__key` values.
                                if (key.startsWith('__')) {
                                    if (!Array.isArray(context[key])) {
                                        // Init array from non array value.
                                        context[key] = [context[key]];
                                    }
                                    if (Array.isArray(dataValue)) {
                                        // Join arrays without duplicating existing items.
                                        const uniqueValues = dataValue.filter(value => !context[key].includes(value));
                                        if (uniqueValues.length) {
                                            context[key] = context[key].concat(uniqueValues);
                                            hasNewData = true;
                                        }
                                    } else if (!context[key].includes(dataValue)) {
                                        // Do not insert existing value.
                                        context[key].push(dataValue);
                                        hasNewData = true;
                                    }
                                }
                            }
                ...
                } else if (isGetVarsMethod(r.method.name)) {
                        ...
                        if (v !== '' && v !== null && ((typeof v === 'string' && !/^\s+$/.test(v)) || typeof v === 'number' || typeof v === 'object')) {
                            ...
                            if (typeof v === 'object' && typeof oldValue === 'object' || Array.isArray(oldValue)) {
                                ...
                                if (isArrayMode) {
                                    ...
                                    // Merge object mode. Prevent mutation.
                                    if (newValueOverrides) {
                                        // Extend existing object.
                                        allResults.vars[key] = Object.assign({}, oldValue, v);
                                    } else {
    ...
    const BIG_CONTEXT = ['readability', '$fulltext', 'decode', 'cheerio'];

    function prepareResultData(uri, result, options) {

        function getSourcesParams(_sources, _sources_params, method_name) {
            for(var key in _sources) {
                var pluginIds = _sources[key];
                ...
                for(var i = 0; i < pluginIds.length; i++) {
                    var plugin = plugins[pluginIds[i]];
                    var params = plugin.methods[method_name];

                    if (!params && method_name === 'getVars' && CONFIG.GET_VARS_METHODS) {
                        // Var may come from an aliased getVars method, e.g. `getContent` -> `content`.
                        var aliasMethod = Object.keys(CONFIG.GET_VARS_METHODS).find(m => CONFIG.GET_VARS_METHODS[m] === key);
                        if (aliasMethod) {
                            params = plugin.methods[aliasMethod];
                        }
                    }

                    if (params && params.filter) {
                        mergedParams = mergedParams.concat(params.filter(param => param in result._debug.data));
                    }
                }

                if (mergedParams.length) {
                    _sources_params[key] = [...new Set(mergedParams)];
                }
            }
        }
        ...
        if (options.debug_sources) {
            ...
            result.allData.forEach(function(d) {
                if (d.data && d.method.name === 'getData') {
                    var plugin = plugins[d.method.pluginId];
                    if (plugin.listed) {
                        for(var key in d.data) {
                            if (typeof plugin.listed === 'boolean' 
                                // listed is string with specific key, or array of string keys.
                                || (typeof plugin.listed === 'string' && plugin.listed === key)
                                || (Array.isArray(plugin.listed) && plugin.listed.includes(key))) {
                                result._debug.data[key] = d.data[key];
                            }
                        }
                    }
                }
            ...
            getSourcesParams(result.meta._sources, result._debug.meta_sources, 'getMeta');
            getSourcesParams(result.vars._sources, result._debug.vars_sources, 'getVars');
        }
        ...
        // Clenaup `allData` from data which can't be stringified.
        // `allData` available only for debug mode.
                ...
                for(var key in r.data) {
                    if (r.data[key]) {
                        if (r.data[key].abortController || BIG_CONTEXT.indexOf(key) > -1) {
                            ...
                            try {
                                // Find circular data by error.
                                JSON.stringify(r.data[key])
                            } catch(ex) {
                                log(`Can't stringify data key="${key}" error=${ex.message}`);
                                r.data[key] = 'BIG_CONTEXT';
                            }
                        }
                    }
    ...
    function getQueryOptionsFromUrl(url) {
        try {
            return getQueryOptionsFromEntries(new URL(url).searchParams);
        } catch(ex) {
    ...
    function generateRegisterFetchFunc(url, options) {
        ...
        options.registerFetchError = function(data) {
            if (options.returnFetchUsage) {
                ...
                // Remove same start fetch record by `source` and `url` only.
                options._fetches = options._fetches.filter(function(item) {
    ...
    function generateProviderOptionsFunc(url, options) {
        ...
        options.getDomainOptions = function(path, defaultValue) {
            ...
            p3. domainOptions:      {robot: {prerender: true}}
            */

            const bits = path.split('.');

            const alternatePath = path.replace(/^[^.]+\./, '_');    // "robot.prerender" -> "_.prerender"
    ...
    * */
    export function run(uri, options, cb) {
        ...
        if (utils.isBlocked(uri, options, cb)) {
            if (options.v === '1.3') {
                cb({
                    code: 'http error',
                    responseCode: 417,
                    messages: ['This domain is ignored based on your settings.']
                });
            } else {
                cb(417);
            }
        ...
        if ((options.debug || options.debug_sources) && !options.totalTimer) {
        ...
        var requiredPlugins = pluginsSet.initialPlugins,
            ...
            asyncMethodCb = function(error, result) {
                ...
                if (hasNewData || error === 'initial' || error === 'lastIteration') {
                    hasRuns = runPluginsIteration(runOpts);
                }
                ...
                if (hasRuns === 0) {
                    ...
                    if (getVarsInfo.pendingMethods) {
                        ...
                        // Run last iteration.
                        return asyncMethodCb('lastIteration');
                    }
                    ...
                    if (!options.mixAllWithDomainPlugin && isDomainPluginsMode && !resultsHasDomainData(requiredPlugins, allResults.allData)) {
                        ...
                        isDomainPluginsMode = pluginsSet.isDomainPluginsMode;
                        getVarsInfo = {}; // Also reset to restart process.

                        // Recursive call with same context to prevent same data fetching.
                        hasRuns = runPluginsIteration({requiredPlugins, context, pluginsUrlMatches, usedMethods, usedParams, usedDomains, getVarsInfo, options, asyncMethodCb});

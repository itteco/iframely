    ...
    * */
    ...
    function generateProviderOptionsFunc(options) {
        ...
        options.getRequestOptions = function(path, defaultValue) {
            ...
            var value = options.getProviderOptions(alternatePath, options.getProviderOptions(path, defaultValue));
            if (
    ...
    var run = core.run = function(uri, options, cb) {
        ...
        var requiredPlugins = pluginsSet.initialPlugins,
            ...
            abortCurrentRequest = function() {
                if (context.htmlparser) {
                    utils.abortRequest(context.htmlparser.request);
                }
            },
            ...
            asyncMethodCb = function(error, result) {
                ...
                if (hasRuns === 0) {
                    ...
                    if (!options.mixAllWithDomainPlugin && isDomainPluginsMode && !resultsHasDomainData(requiredPlugins, allResults.allData)) {
                        ...
                        sysUtils.log('   -- fallback from domain to generic', usedDomains, uri);

                        if (options.events && options.events.emit) {
                            var usedDomainsList = _.keys(usedDomains);
                            process.nextTick(function() {
                                options.events.emit('fallback-from-domain-to-generic', usedDomainsList, uri);
                            });
                        }

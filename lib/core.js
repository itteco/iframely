    ...
    * */
    ...
    function prepareResultData(result, options) {
        ...
        for(var i = 0; i < links.length;) {
            ...
            // Remove canonical links.
            if (canonical && link.href && typeof link.href === 'string') {
        ...
        var mainLink = htmlUtils.findMainLink(result, options);
        if (mainLink) {
            if (mainLink.html) {
                result.rel = mainLink.rel;
                result.html = mainLink.html;
            }
        }
    ...
    * */
    var run = core.run = function(uri, options, cb) {
        ...
        var usedParams = {
            ...
            // This makes 'whitelistRecord' not mandatory. Function with that param will be called anyway.
            // So always need to check in plugins: if (whitelistRecord === null)
        ...
        var requiredPlugins = pluginsSet.initialPlugins,
            ...
            context = {
                ...
                options: options,
                request: request,
                whitelistRecord: options.getWhitelistRecord && options.getWhitelistRecord(uri)
            },

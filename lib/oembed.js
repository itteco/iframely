...
if (CONFIG.customVendorMapping) {
    var rawCustomVendorMapping = fs.readFileSync(CONFIG.customVendorMapping);
    var customVendorMapping = JSON.parse(rawCustomVendorMapping);
}
...
export function getOembed(uri, data, options) {
    ...
    // don't attach scripts defined in the global meta data in amp output mode
    if(!options.amp) {
        if (data.meta.script) {
            oembed.scripts.push({ src: data.meta.script });
        }
        if (data.meta.inline_script) {
            oembed.scripts.push({ innerHTML: data.meta.inline_script });
        }
    }

    if (link && link.script) {
        oembed.scripts.push(link.script);
    }

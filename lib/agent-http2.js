...
Http2Response.prototype.close = function() {
    // Proxy stream 'close' method.
    // Fixes 'FLOW' errors.
    if (!this.clientHttp2Stream.closed) {
        this.clientHttp2Stream.close();
    }
}
...
Http2Agent.prototype.getHttp2Session = function(options, socket) {
    ...
    if (!session) {
        ...
        this.http2SessionsReuse[name]++;
        console.log('---- reused sessions, total count', Object.keys(this.http2SessionsReuse).length)
        for (var key in this.http2SessionsReuse) {
            var count = this.http2SessionsReuse[key];
            if (count > 0) {
                console.log('  -- reused session for', key, ':', count + 1, 'times');
            }
        }
    }
...
Http2Agent.prototype.addSession = function(name, session) {
    ...
    this.http2SessionsReuse[name] = 0;
...
Http2Agent.prototype.removeSession = function(name, session) {
    ...
    delete this.http2SessionsReuse[name];
    try {
        if (!session.closed) {
            session.close();
        }

...
Http2Agent.prototype.addRequest = function(req, host, port, localAddress) {
    ...
    this.createConnection(
        ...
        function(socket) {
            ...
            console.log('-- https1 fallback', info.host);
...
Http2Agent.prototype.getTlsSocketConnected = function(options, cb) {
    ...
    // Create new socket.
    var socket = tls.connect(_.extend({}, options, { 
        ALPNProtocols: ['h2', 'http/1.1'],
        session: session    // Reuse session.
    }), function() {
        // TODO: multiple requests on same domain will create two sessions.
        that.tlsSocketSessions[name] = socket.getSession();
        cb(socket);
    });
...
Http2Agent.prototype.getHttp2Session = function(options, socket) {
    ...
    if (!session) {
        ...
        // Create new session.
        console.log('-- http2.connect', authority);
...
Http2Agent.prototype.onHttp2Session = function(req, http2Session) {
    ...
    var headers = {[http2.constants.HTTP2_HEADER_PATH]: req.path};
    _.extend(headers, req.getHeaders());

(function(engine) {
    ...
    var MEMCACHED_MAX_DATA_SIZE = 1024 * 1024 - 128;  // Megabyte. -128 - test limit.
    ...
    engine.set = function(_key, data, options) {
        ...
        if (storedData && storedData.length > MEMCACHED_MAX_DATA_SIZE) {
            ...
            sysUtils.log('   -- Memcached handled set error ' + _key + ': The length of the value is greater than ' + MEMCACHED_MAX_DATA_SIZE);

        } else {

            memcached.set(key, storedData, options && options.ttl || CONFIG.CACHE_TTL, function(error){
                if (error) {
                    sysUtils.log('   -- Memcached set error ' + _key + ', value length ' + storedData.length + ', error ' + error);
                }
            });
        }
    };

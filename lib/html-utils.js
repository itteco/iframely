(function() {
    ...
    var defaultPaddingBottom = 56.25; // CONFIG.DEFAULT_ASPECT_RATIO ?
    ...
    function wrapContainer($element, data, options) {
        ...
        var hasMaxWidth = media && (media["max-width"] || media["min-width"] || media["width"] || media["min-height"]);
        ...
        if (!omitInlineStyles) {
            ...
            if (!media || (!media.height && !media["aspect-ratio"])) {
                $container.css('padding-bottom', defaultPaddingBottom + '%');
            }
        ...
        if (media) {
            ...
            if (hasMaxWidth) {
                ["max-width", "min-width", "max-height"].forEach(function(attr) {
                    if (media[attr]) {
                        $widthLimitContainer.css(attr, media[attr] + 'px');
                    }
                });
            }
        }

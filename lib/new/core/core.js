(function(core) {
    ...
    function findPluginMethods(pluginId, loadedParams, pluginsUrlMatches, usedMethods, usedParams, methods, scannedPluginsIds, mandatoryParams, mandatoryPlugins) {
        ...
        * This dict used to determine new params (mandatory params), which could be used in some new plugins (mandatory plugins).
        ...
        *
        *
        * methods - result methods to run list.
        * methods = [{
        *   pluginId: pluginId,
        *   name: methodName,
        *   handle: <function>
        ...
        for(var i = 0; i < mandatoryParams.length; i++) {
            ...
            if (pluginsIds) {
                for(var j = 0; j < pluginsIds.length; j++) {
                    ...
                    // TODO: select matched domain plugin. Not only by matched re.
                    if (!(pluginId in foundPluginsDict) && (!plugin.domain || plugin.id in pluginsUrlMatches)) {

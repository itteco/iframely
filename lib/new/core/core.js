(function(core) {
    ...
    * */
    ...
    function prepareResultData(allData) {
        var links = allData.links;
        var canonical = allData.meta.canonical;
        if (canonical) {
    ...
    function findRedirectError(result) {
        if (result) {
            for(var i = 0; i < result.length; i++) {
                var r = result[i];
                if (r.error && r.error.redirect) {
                    return r.error.redirect;
                }
            }
        }
    }
    ...
    function findResponseStatusCode(result) {
        if (result) {
            for(var i = 0; i < result.length; i++) {
                var r = result[i];
                if (r.error && r.error.responseStatusCode) {
                    return r.error.responseStatusCode;
                }
            }
        }
    }
...
*/
    ...
    run('http://habrastorage.org/getpro/habr/post_images/536/a53/e6a/536a53e6a2f2f9025b1447056d1b091c.jpg', {debug: false}, function(error, result) {

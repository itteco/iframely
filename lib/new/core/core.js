(function(core) {
            ...
            for(var i = 0; i < pluginsList.length; i++) {
                ...
                if (plugin.domain) {
                    ...
                    if (match) {
                        ...
                        // TODO: !!! do not store match if no match.
                        continue;
                    }
    ...
    * */
    ...
    function useResult(usedMethods, context, allResults, result) {
    ...
    function resultsHasDomainData(allResults) {
        var hasDomainData = false;
        for(var i = 0; i < allResults.length && !hasDomainData; i++) {
            var r = allResults[i];
    ...
    function run(uri, options, cb) {
        ...
        var requiredPlugins = pluginsSet.initialPlugins,
            ...
            asyncMethodCb = function(error, result) {
                ...
                //console.log(' - call result:', result.map(function(r) {return r.method.pluginId;}).join(', '));

                // Gather results.
                useResult(usedMethods, context, allResults, result);
                ...
                if (hasRuns === 0) {
                    ...
                    if (!options.mixAllWithDomainPlugin && usedDomains && !resultsHasDomainData(allResults)) {

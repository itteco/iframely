    ...
    function doRealRequest(secondAttempt) {
        ...
        request(utils.prepareRequestOptions(options, iframely_options), function(error, response, data) {
            ...
            if (error) {
                ...
                // Retry on ECONNRESET.
                if (!secondAttempt && error.code === 'ECONNRESET') {
                    sysUtils.log('   -- request.js ECONNRESET first', url);
                    process.nextTick(function() {
                        doRealRequest(true);
                    });
                    return;
                } else if (secondAttempt && error.code === 'ECONNRESET') {
                    sysUtils.log('   -- request.js ECONNRESET second', url);
                }
            }
            ...
            prepareResult(error, response, data, function(preparedError, preparedData) {
                ...
                if (allowCache) {
                    ...
                    if (!doCaching) {
                        sysUtils.log(' -- skipping request.js caching for', options.url || options.uri)
                    }
                } else {
            ...
            if (!error && data) {
                ...
                if (allowCache) {
                    ...
                    if (!doCaching) {
                        ...
                        sysUtils.log(' -- skipping request.js cache for', options.url || options.uri)
                        doRealRequest();
                        return;
                    }
                }

(function(whitelist) {
    ...
    function loadLastWhitelist() {
        ...
        if (filename && filename != currentWhitelistFilename) {
            ...
            console.log('------------------------------------------------------');
            console.log('No domains DB whitelist detected...');
            console.log('You could extend Iframely coverage to 1000+ domains');
            console.log('Get your copy of Iframely DB', 'http://iframely.com/qa');
            console.log('------------------------------------------------------');
            addWildcard();
        }
    ...
    function loadWhitelistUrl() {
        ...
        if (!currentWhitelistFilename && CONFIG.WHITELIST_URL && CONFIG.WHITELIST_URL_RELOAD_PERIOD) {
            ...
            if (CONFIG.WHITELIST_URL.search(/sample\.json/)) {
                console.log("Loading whitelist from " + CONFIG.WHITELIST_URL + ". Get full version at http://iframely.com/qa/buy");
            } else {
            ...
            request({
                ...
                if (error) {
                    console.error('Error loading whitelist from ' + CONFIG.WHITELIST_URL);
                } else {
                    applyParsedWhitelist(newWhitelist);
                }

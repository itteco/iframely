(function(whitelist) {
    ...
    function loadWhitelistUrl() {
        ...
        if (!currentWhitelistFilename && CONFIG.WHITELIST_URL && CONFIG.WHITELIST_URL_RELOAD_PERIOD) {
            ...
            if (whitelistObjectEtag) {
                options.headers = {
                    'X-If-None-Match': whitelistObjectEtag,
                    'If-None-Match': whitelistObjectEtag
                };
            }

            request(options, function(error, r, newWhitelist) {
                ...
                if (error) {
                    ...
                    whitelistObjectEtag = r.headers['etag'] || r.headers['x-etag'];
                    applyParsedWhitelist(newWhitelist);
                }

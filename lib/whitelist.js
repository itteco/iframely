    ...
    function extractDomainPatterns(uri, disableWildcard) {
        ...
        if (bits.length > 2) {
            for(var i = 0; i < bits.length - 1; i++) {
                var d = bits.slice(i);
                if (d.length === 2 && (d[0] === 'co' || d[0] === 'com')) {
                    // Skip '*.co.uk' and '*.com.ua'.
                } else {
                    patterns.push('*.' + d.join('.'));
                }
            }
        } else {
            patterns.push('*.' + domain);
        ...
        patterns.forEach(function(pattern) {
            ...
            var d = bits.slice(0, -1);

            if (d.length > 1) {
                // Skip last 'co' or 'com' domain.
        ...
        return patterns.concat(petterns2);
    }

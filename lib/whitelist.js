    ...
    export function findWhitelistRecordFor(uri, options) {
        ...
        var overrideDomains = options && options.overrideDomains;

        if (!whitelistObjec && !overrideDomains) {
            return null;
        }
        ...
        while(!record && i < patterns.length) {
            ...
            if (overrideDomains && overrideDomains[pattern]) {
                record = overrideDomains[pattern];
            } else {
                record = whitelistObject && whitelistObject.domains[pattern];
            }
            if (record) {
                record = _.extend({
                    domain: patterns[i],
                    ...
                    isDefault: pattern === "*" // true for wildcard from config
                }, record);

module.exports = {
    ...
    getData: function(url, video_src, twitter) {
        ...
        if (stream
            &&  /traffic\.libsyn\.com\/preview\/([a-zA-Z0-9\-]+)\//i.test(stream)
            && /^https?:\/\/html5\-player\.libsyn\.com\/embed(\/episode\/id\/\d+)/i.test(video_src)
            && /^https?:\/\/[a-z\.]+\//i.test(url)) {

            return {
                __promoUri: {                    
                    url: 'http://' + stream.match(/traffic\.libsyn\.com\/preview\/([a-zA-Z0-9\-]+)\//i)[1] 
                        + '.libsyn.com/'
    ...
    // http://hannity.rsvmedia.com/donald-trump-jr-816
